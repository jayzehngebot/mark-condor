...
export default function Sources() {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-500 p-10">
            <h1 className="text-4xl text-center mt-4 text-slate-400 mb-6">Sources <span className="text-slate-600">(of Inspiration)</span></h1>
            <div className="flex flex-row flex-wrap items-center justify-center w-full">
            {sources.map((source: Source) => (
                <Basecard key={source.id} id={source.id} title={source.title} description={source.description} source_url={source.source_url} />
            ))}
            </div>
        </div>
    )
