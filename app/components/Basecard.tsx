...
const Basecard: React.FC<BasecardProps> = ({ id, title, description, source_url, wide, tags, image, openInNewTab, subhed }) => {
    return (
        <div id={`source_${id}`} className={`bg-slate-900 rounded-lg shadow-md ${wide ? 'w-full' : 'w-full sm:w-1/2 md:w-1/4 lg:w-1/4'} h-32 sm:h-40 m-2 mb-8 mt-0`} style={{ backgroundImage: `url('/backgrounds/${id}.png')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
